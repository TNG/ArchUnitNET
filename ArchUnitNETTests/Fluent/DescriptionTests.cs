...
namespace ArchUnitNETTests.Fluent
{
    ...
    {
        private readonly IArchRule _descriptionTestRule = Classes().That().HaveNameStartingWith("test").And()
            .AreNotNested().Should().BeAbstract().OrShould().NotBeAbstract().And(Types().Should().BeNested()).Or()
