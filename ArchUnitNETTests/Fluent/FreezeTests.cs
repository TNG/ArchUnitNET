...
namespace ArchUnitNETTests.Fluent
{
    public class FreezeTests
    {
        ...
        private readonly IArchRule _frozenRule =
            Types().That().Are(typeof(Violation), typeof(Violation2)).Should().NotBePrivate();

        private readonly IArchRule _frozenRule2 = Types().That().Are(typeof(Violation)).Should().BeProtected();

        private readonly IArchRule _failingFrozenRule =
            Types().That().Are(typeof(Violation), typeof(Violation2)).Should().BePublic();

        private readonly IArchRule _frozenSliceRule =
            SliceRuleDefinition.Slices().Matching("TestAssembly.Slices.(**)").Should()
                .NotDependOnEachOther();

        private readonly IArchRule _failingFrozenSliceRule =
        ...
        public void PassFrozenRules()
        {
            ...
            Freeze(_frozenSliceRule).Check(StaticTestArchitectures.ArchUnitNETTestAssemblyArchitecture);
        }
        ...
        {
            Assert.Throws<FailedArchRuleException>(() => Freeze(_failingFrozenRule).Check(Architecture));
            Assert.Throws<FailedArchRuleException>(() => Freeze(_failingFrozenSliceRule).Check(StaticTestArchitectures.ArchUnitNETTestAssemblyArchitecture));
        }
        ...
        public void FailFrozenRuleUsingXmlViolationStore()
        {
            ...
            Assert.Throws<FailedArchRuleException>(() =>
                Freeze(_failingFrozenSliceRule, new XmlViolationStore()).Check(StaticTestArchitectures.ArchUnitNETTestAssemblyArchitecture));
        }
