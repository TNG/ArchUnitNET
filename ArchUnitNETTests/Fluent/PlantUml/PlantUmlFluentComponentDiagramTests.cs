...
namespace ArchUnitNETTests.Fluent.PlantUml
{
    public class PlantUmlFluentComponentDiagramTests
    {
        private static readonly Architecture Architecture = new ArchLoader()
            .LoadAssembly(typeof(PlantUmlFluentComponentDiagramTests).Assembly)
            .Build();
        ...
        public void ComponentDiagramFromSlicesTest()
        {
            ...
            var uml1 = ComponentDiagram()
                .WithDependenciesFromSlices(sliceRule, Architecture)
                .AsString();
            var uml2 = ComponentDiagram()
                .WithDependenciesFromSlices(sliceRule.GetObjects(Architecture))
                .AsString();
            Assert.NotEmpty(uml1);
            Assert.NotEmpty(uml2);

            var arch1 = new ArchLoader().LoadAssembly(typeof(Architecture).Assembly).Build();
            var sliceRule1 = SliceRuleDefinition
                .Slices()
                .MatchingWithPackages("ArchUnitNET.(*).(*).(*)");
            const string path = "../../../Fluent/PlantUml/Test.puml";
            var g = new GenerationOptions() { C4Style = true, LimitDependencies = true };
            ComponentDiagram()
                .WithDependenciesFromSlices(sliceRule1.GetObjects(arch1), g)
                .WriteToFile(path);
            File.Delete(path);
        }
        ...
        {
            var typeRule = ArchRuleDefinition
                .Types()
                .That()
                .Are(typeof(PlantUmlFluentComponentDiagramTests));
            var uml1 = ComponentDiagram()
                .WithDependenciesFromTypes(typeRule, Architecture)
                .AsString(new RenderOptions { OmitClassFields = true });
            var uml2 = ComponentDiagram()
                .WithDependenciesFromTypes(typeRule.GetObjects(Architecture))
                .AsString(new RenderOptions { OmitClassFields = true });
            var uml3 = ComponentDiagram()
                .WithDependenciesFromTypes(
                    typeRule,
                    Architecture,
                    new GenerationOptions { IncludeDependenciesToOther = true }
                )
                .AsString();
            var uml4 = ComponentDiagram()
                .WithDependenciesFromTypes(
                    typeRule.GetObjects(Architecture),
                    new GenerationOptions { IncludeDependenciesToOther = true }
                )
                .AsString();
            Assert.NotEmpty(uml1);
