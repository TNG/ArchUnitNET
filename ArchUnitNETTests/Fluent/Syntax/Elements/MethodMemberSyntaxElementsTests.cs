...
namespace ArchUnitNETTests.Fluent.Syntax.Elements
{
    ...
    public class MethodMemberSyntaxElementsTests
    {
        ...
        public async Task DoNotHaveReturnTypeTest()
        {
            ...
            // A method whose return type is in the empty set is vacuously not in it, so the negation holds
            // for every input. The IObjectProvider overload formats its own description via
            // ObjectProvider.FormatDescription, so it does not read identically to the list overloads.
            helper.AddSnapshotHeader("Empty arguments");
            should = MethodMembers().That().Are(helper.MethodReturningRegularClass).Should();
