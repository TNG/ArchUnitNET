...
{
    internal class PredicateManager<T> : IHasDescription
        where T : ICanBeAnalyzed
    {
        ...
        public IEnumerable<T> GetObjects(Architecture architecture)
        {
            ...
            return _predicateElements.Aggregate(
                filteredObjects,
                (current, predicateElement) =>
                    predicateElement.CheckPredicate(current, objects, architecture)
            );
        }
        ...
        public void SetCustomDescription(string description)
        {
            ...
            _predicateElements.ForEach(predicateElement =>
                predicateElement.SetCustomDescription("")
            );
        ...
        public override int GetHashCode()
        {
            ...
            {
                var hashCode =
                    _basicObjectProvider != null ? _basicObjectProvider.GetHashCode() : 0;
                return _predicateElements.Aggregate(
                    hashCode,
                    (current, predicateElement) =>
                        (current * 397)
                        ^ (predicateElement != null ? predicateElement.GetHashCode() : 0)
                );
            }
        }

#pragma warning disable 693
        private class PredicateElement<T> : IHasDescription
            where T : ICanBeAnalyzed
        {
                ...
                ?? (
                    _predicate == null
                        ...
                        : (
                            _logicalConjunction.Description
                            + " "
                            + _predicate.GetShortDescription()
                            + " "
                            + _reason
                        ).Trim()
                );
            ...
            public IEnumerable<T> CheckPredicate(
                IEnumerable<T> currentObjects,
                IEnumerable<T> allObjects,
            ...
            public override int GetHashCode()
            {
                unchecked
                {
                    ...
                    hashCode =
                        (hashCode * 397) ^ (_predicate != null ? _predicate.GetHashCode() : 0);
