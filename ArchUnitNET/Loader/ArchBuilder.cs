...
namespace ArchUnitNET.Loader
{
    internal class ArchBuilder
    {
        ...
        public void AddAssembly(
            [NotNull] System.Reflection.Assembly systemAssembly,
            bool isOnlyReferenced,
            [CanBeNull] IEnumerable<AssemblyName> moduleReferences
        )
        {
            var references = moduleReferences?.Select(reference => reference.FullName).ToList();
            if (!_assemblyRegistry.ContainsAssembly(systemAssembly.FullName))
            {
                ...
                // TODO: CollectAssemblyAttributes
                // _loadTaskRegistry.Add(
                //     typeof(CollectAssemblyAttributes),
                //     new CollectAssemblyAttributes(assembly, systemAssembly, _typeFactory)
                // );
            }
        }

        public void LoadTypesForModule(Module module, string namespaceFilter)
        {
            ...
            module
                ...
                .Where(t =>
                    ...
                    && t.GetCustomAttribute<System.Runtime.CompilerServices.CompilerGeneratedAttribute>()
                        == null
                    && t.CustomAttributes.All(att =>
                        att.AttributeType.FullName
                        != "Microsoft.VisualStudio.TestPlatform.TestSDKAutoGeneratedCode"
                    )
                    && RegexUtils.MatchNamespaces(namespaceFilter, t.Namespace)
                )
                .ForEach(t =>
                {
                    var typeInstance = _typeFactory.GetOrCreateTypeFromSystemType(t, false);
                    if (!_architectureTypes.ContainsKey(t.AssemblyQualifiedName))
                    {
                        _architectureTypes.Add(t.AssemblyQualifiedName, typeInstance.Type);
                    }
                });

            // TODO: AddTypesToNamespace
            // _namespaceRegistry
